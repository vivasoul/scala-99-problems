...
object P28 {
	def lsort[A](xs: List[List[A]]): List[List[A]] = xs.sortBy(_.size)
	
	def lsortFreq[A](xs: List[List[A]]): List[List[A]] = xs.sortBy(x => xs.count(_.size == x.size))
}

P28.lsort(List(List('a, 'b, 'c), List('d, 'e), List('f, 'g, 'h), List('d, 'e), List('i, 'j, 'k, 'l), List('m, 'n), List('o)))
